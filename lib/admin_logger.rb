class AdminLogger
  ...
  def step_results topic, game_id, params = {}
    ...
    roles_data = statements.visible_for_buf(rebuild_visible_for(state.stage)).inject({}){|r, v| r.merge(v.value.inspect => (v.player_contribution.inject({}){|res, (key, val)| res.merge(key => ('%.03f' % val)) } )) }
